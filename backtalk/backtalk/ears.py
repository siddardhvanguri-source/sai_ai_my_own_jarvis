...
def transcribe(pcm: np.ndarray) -> str:
    """int16 mono 16kHz -> text. Tries Groq Cloud Whisper first for ultra-low latency,
    falling back to local faster-whisper."""
    groq_key = (CFG.get("groq_api_key") or os.environ.get("GROQ_API_KEY") or "").strip()
    if "..." in groq_key:
        groq_key = ""
    if groq_key:
        try:
            ...
            log(f"[ears] Groq STT failed ({e}) — falling back to local model")
        ...
        with _open_mic() as stream:
            while True:
                ...
                if gate and gate():
                    ...
                    ring.clear()
                    continue
                # Hybrid VAD + RMS energy detection for laptop mic arrays
                rms = np.sqrt(np.mean(mono.astype(float)**2))
                is_speech = False
                try:
                    is_speech = self.vad.is_speech(mono.tobytes(), RATE)
                except Exception:
                    pass
                if not is_speech and rms > 60.0:
                    is_speech = True
...
def play_chime(chime_type: str = "start"):
    """Silent — all synthetic beeps and chimes disabled."""
    pass

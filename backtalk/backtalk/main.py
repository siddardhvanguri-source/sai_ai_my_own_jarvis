    ...
    try:
        ...
        while True:
            ...
            if mic_fut is not None and mic_fut in done:
                ...
                if text:
                    wake = (CFG.get("wake_word") or "sai").lower().strip()
                    if wake and _MIC["mode"] == "open":
                        text_low = text.lower()
                        # Support phonetic variations of Sai (Whisper may transcribe Sai as sigh/cy)
                        wake_aliases = [wake, "hey " + wake, "hi " + wake, "ok " + wake]
                        if wake == "sai":
                            wake_aliases.extend(["sigh", "hey sigh", "cy", "hey cy", "psy"])
                        ...
                        if matched_alias:
                            idx = text_low.find(matched_alias)
                            cmd = text[idx + len(matched_alias):].strip(" ,:.-!?")
                            if not cmd:
                                cmd = "Hello Sai"
                            log(f"[wake] '{matched_alias}' triggered with command: {cmd!r}")
                            if not await handle(cmd):
                                return
                        else:
                            log(f"[ears] ignored room audio (wake word '{wake}' not spoken): {text!r}")
                    else:
